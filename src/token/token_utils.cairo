...
fn transfer(data_store: IDataStoreDispatcher, token: ContractAddress, receiver: ContractAddress, amount: u256) {
    ...
    let success0 = IERC20Dispatcher { contract_address: token }.transfer(receiver, amount);
    if (success0 == true) {
        return;
    }
    ...
    // TODO: emit reverted event with revert reason

    // throw error if transfer fails
    panic(array![BankError::TOKEN_TRANSFER_FAILED]);
}
