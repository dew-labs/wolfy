...
const saveOrRemovePosition = async (
    event: ParsedSatoruEvent<SatoruEvent.DepositCreated>,
    tx: Transaction,
    block: FullBlock
) => {
...
const saveTradeHistory = async (
    ...
    const { key, account, market, initial_long_token_amount, initial_short_token_amount } = event;
    ...
    Object.assign(tradeHistory, {
        ...
        is_long: true,
        deposit_long_token_amount: cairoIntToBigInt(initial_long_token_amount),
        deposit_short_token_amount: cairoIntToBigInt(initial_short_token_amount),
