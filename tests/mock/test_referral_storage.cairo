...
fn given_initialize_when_already_intialized_then_fails() {
    let (_, _, data_store, event_emitter, referral_storage, _, market_factory) = setup();
...
fn given_normal_conditions_when_setting_handler_from_storage_than_work() {
    let (caller_address, _, data_store, _, referral_storage, _, market_factory) = setup();
...
fn given_handler_not_set_than_fails() {
    let (_, _, data_store, _, referral_storage, _, market_factory) = setup();
...
fn setup() -> (
    ...
    return (caller_address, role_store, data_store, event_emitter, referral_storage, governable, market_factory);
}
