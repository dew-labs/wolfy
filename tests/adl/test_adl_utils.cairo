...
fn given_normal_conditions_when_set_latest_adl_block_then_works() {
    // Setup
    let (_caller_address, _role_store, data_store, _, _, _market_utils) = setup();
...
fn given_normal_conditions_when_emit_adl_state_updated_then_works() {
    // Setup
    let (caller_address, _role_store, _data_store, event_emitter, _, _market_utils) = setup();
...
fn given_small_block_number_when_update_adl_state_then_fails() {
    // Setup
    let (_caller_address, _role_store, data_store, event_emitter, oracle, market_utils) = setup();
    let market = 'market'.try_into().unwrap();
    let is_long = false;
    let block_value = 1234_u64;
    adl_utils::set_latest_adl_block(data_store, market, is_long, block_value);
    ...
    let block_numbers = array![10_u64, 11_u64];
    adl_utils::update_adl_state(data_store, event_emitter, oracle, market, is_long, block_numbers.span(), market_utils);
...
fn given_non_valid_position_when_create_adl_order_then_fails() {
    ...
    let (_caller_address, _role_store, data_store, event_emitter, _oracle, _market_utils) = setup();
...
fn given_normal_conditions_when_update_adl_state_then_works() {
    // Setup
    let (caller_address, role_store, data_store, event_emitter, oracle, market_utils) = setup();
    // TODO
...
fn setup() -> (
    ...
    (caller_address, role_store, data_store, event_emitter, oracle, market_utils)
}
