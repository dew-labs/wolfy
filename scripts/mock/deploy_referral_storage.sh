...
command_output=$(starkli declare ../../target/dev/satoru_ReferralStorage.sierra.json --network=sepolia --compiler-version=2.6.0 --account $1 --keystore $2)

from_string="Class hash declared:"
class_hash="${command_output#*$from_string}"

# Deploy the contract using the extracted class hash
starkli deploy $class_hash $3 --network=sepolia --account $1 --keystore $2
